...
module.exports = async function({ channel, format, args }) {
    ...
    if (args.length < 2 || args.length > 3 || isNaN(args[1]) || isNaN(args[2])) return sendFormatErr(channel, format)
    ...
    for (var i = 0; i < noOfSummons; i++) {
        fields.push({
            name:  `${args[0]}${i + 1}`,
            value: `${args[2]}/${args[2]}\n    Initative: ${roll(["d20", args[1]], channel, "")}`,
            inline: false
        })
        ...
        channel.messages.get(latestBattle.id).edit({ embeds: newEmbedList })
    }
}